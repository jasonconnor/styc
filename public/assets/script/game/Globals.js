...
function updateStats(s, p, e, postBattle = false) {
    ...
    statsDisplay.innerHTML += "<br>Your HP: <player-hp>" + p.hp + 
        "</player-hp>/<player-hp>" + p.maxHP + "</player-hp>";
    statsDisplay.innerHTML += "<br><bad-guy>" + e.name + 
        "</bad-guy>'s HP: <enemy-hp>" + e.hp + 
        "</enemy-hp>/<enemy-hp>" + e.maxHP + "</enemy-hp>";
    if (!postBattle)
